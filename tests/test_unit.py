...
class FunctionalTests(unittest.TestCase):
    ...
    def test_read_write_file(self):
        fs = self.make_one()
        self.assertEqual(fs.hash(), b"4b825dc642cb6eb9a060e54bf8d69288fbee4904")
        ...
        self.assertEqual(fs.hash("foo"), b"e965047ad7c57865823c7d992b1d046ea66edf78")
    ...
    def test_read_write_file_in_subfolder(self):
        ...
        self.assertEqual(fs.hash("foo"), b"4b825dc642cb6eb9a060e54bf8d69288fbee4904")
        with fs.open("foo/bar", "wb") as f:
            fprint(f, b"Hello")
        with fs.open("foo/bar", "rb") as f:
            self.assertEqual(f.read(), b"Hello\n")
        self.assertEqual(fs.hash("foo"), b"c57c02051dae8e2e4803530c217ac38121f393d3")
    ...
    def test_open_edge_cases(self):
        ...
        with fs.open("bar", "wb") as f:
            ...
            with self.assertRaises(ValueError) as cm:
                transaction.commit()
            self.assertEqual(
                str(cm.exception), "Cannot commit transaction with open files."
            )
        transaction.abort()
    ...
    def test_modify_file(self):
        ...
        with fs.open("foo", "wb") as f:
            ...
            self.assertEqual(fs.hash("foo"), b"c564dac563c1974addaa0ac0ae028fc92b2370f1")
        self.assertEqual(fs.open("foo", "rb").read(), b"Hello!\n")
        self.assertEqual(fs.hash("foo"), b"10ddd6d257e01349d514541981aeecea6b2e741d")
    ...
    def test_mv(self):
        ...
        self.assertEqual(open(j(self.tmp, "one", "a", "bar"), "rb").read(), b"Hello!")

        transaction.commit()
        self.assertFalse(fs.exists("one/b/foo"))
        self.assertEqual(fs.open("one/a/bar", "rb").read(), b"Howdy!")
        self.assertFalse(pexists(j(self.tmp, "one", "b", "foo")))
        self.assertEqual(open(j(self.tmp, "one", "a", "bar"), "rb").read(), b"Howdy!")
    ...
    def test_branch_and_then_merge(self):
        ...
        commit = (
            _check_output(["git", "cat-file", "-p", "HEAD^{commit}"], cwd=self.tmp)
            .decode("ascii")
            .split("\n")
        )
